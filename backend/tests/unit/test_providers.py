...
@pytest.mark.asyncio
async def test_stream_gemini_yields_chunks():
    chunk1 = MagicMock()
    chunk1.choices = [MagicMock(delta=MagicMock(content="Hello"))]
    chunk2 = MagicMock()
    chunk2.choices = [MagicMock(delta=MagicMock(content=" Gemini"))]
    chunk3 = MagicMock()
    chunk3.choices = [MagicMock(delta=MagicMock(content=None))]
    ...
    with patch("app.llm.providers._get_gemini", return_value=mock_client):
        from app.llm.providers import stream_gemini
        chunks = []
        async for chunk in stream_gemini([{"role": "user", "content": "test"}], "system"):
            chunks.append(chunk)
